...
const ChatLabel = ({openMenu, setOpenMenu, id, name}) => {
  ...
  try {
    ...
    if (data.success) {
      await fetchUsersChats();             // pastikan list ke-refresh
...
>
  ...
  >
    ...
    {/* STOP PROPAGATION DI PANEL */}
    <div
      onClick={(e) => e.stopPropagation()}
      className={`absolute ${openMenu.id === id && openMenu.open ? 'block' : 'hidden'} -right-36 top-6 bg-gray-700 rounded-xl w-max p-2`}
    >
      {/* STOP PROPAGATION DI ITEM */}
      <div onClick={(e)=>{ e.stopPropagation(); renameHandler(); }} className="flex items-center gap-3 hover:bg-white/10 px-3 py-2 rounded-lg">
        <Image src={assets.pencil_icon} alt="" className="w-4" />
        <p>Rename</p>
      </div>
      <div onClick={(e)=>{ e.stopPropagation(); deleteHandler(); }} className="flex items-center gap-3 hover:bg-white/10 px-3 py-2 rounded-lg">
